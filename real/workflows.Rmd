...
Error seems to be limited to below 10% in most cases, which is acceptable for log-fold change considerations.
Without pre-clustering, maximum errors are around 20%, and can be even higher than 50% with library size normalization.
...
This is probably caused by coercion of multiple small clusters into a single level in `clust` to satisfy the minimum size in `quickCluster()`.
As a result, the non-DE majority is violated.
I consider this a pathological case as it involves many small clusters that are very different, such that there is not much information to share.
