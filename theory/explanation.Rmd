...
# Advantages compared to `computeSumFactors()`

- `quickSumFactors()` does not solve a linear system.
...
This is no longer necessary with `quickSumFactors()` due to the pairwise comparisons to the reference cell $r$.
...
In most scenarios, `quickSumFactors()` is more accurate than `computeSumFactors()` without preclustering.
...
Indeed, most of the loss of performance from `quickSumFactors()` is due to a lower-than-strictly-necessary $k$.

The counter-argument is that `quickSumFactors()` is heavily dependent on correct nearest neighbor detection.
By comparison, `computeSumFactors()` is robust to mis-clustering as each pool-based size factor is estimated using the median.
While this is true, the bias concentration seems to offset a lot of the purported robustness benefits from `computeSumFactors()`.
Even if `quickSumFactors()` does not dominate `computeSumFactors()` completely, it is more practical - and definitely preferable to library size normalization. 
