...
One could imagine that composition biases would yield distorted estimates of the normalized expression values.
...
Previous versions of `quickCluster()` used a minimum size of 200 cells, but we have now reduced this to 20 cells.
In pratice, the inaccuracy from coercing multiple distinct clusters together (and violating the non-DE assumption) is worse than the imprecision from having insufficient cells.
By reducing the minimum number of cells, we avoid the former scenario at the cost of dealing with the latter.
...
This weakens some properties of the pseudo-cell (stability from the law of large numbers, convergence to normality via the CLT). 
